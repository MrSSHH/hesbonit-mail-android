...
class PdfUtilsTest {
    ...
    private fun createTestJpeg(width: Int = 400, height: Int = 300, color: Int = Color.RED): File {
    ...
    @Test
    fun createPdfFromImages produces a non-empty pdf file for a single page() {
        val imageFile = createTestJpeg()

        val pdfFile = PdfUtils.createPdfFromImages(context, listOf(imageFile))
    ...
    @Test
    fun createPdfFromImages produces one A4 page per input image, in order() {
        val imageFiles = listOf(
            createTestJpeg(color = Color.RED),
            createTestJpeg(color = Color.GREEN),
            createTestJpeg(color = Color.BLUE)
        )

        val pdfFile = PdfUtils.createPdfFromImages(context, imageFiles)
    ...
    @Test
    fun createPdfFromImages throws for an empty list() {
        try {
            PdfUtils.createPdfFromImages(context, emptyList())
            fail("Expected an IllegalArgumentException for an empty page list")
    ...
    @Test
    fun createPdfFromImage single-image wrapper still produces a single-page pdf() {
        val imageFile = createTestJpeg()
    ...
    fun calculateInSampleSize returns power of two when image exceeds target() {
        ...
        assertEquals(0, sampleSize and (sampleSize - 1)) // power-of-two check
    }
    ...
    @Test
    fun otateBitmapIfNeeded swaps dimensions for a 90-degree exif orientation() {
    ...
    @Test
    fun otateBitmapIfNeeded leaves bitmap unchanged for normal exif orientation() {
        val imageFile = createTestJpeg(width = 400, height = 300)
        val original = Bitmap.createBitmap(400, 300, Bitmap.Config.ARGB_8888)
